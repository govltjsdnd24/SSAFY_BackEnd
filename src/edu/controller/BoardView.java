...
//		2. 1의 글번호를 이용하여 Dao의 viewArticle()메소드에 전달하고 DB에서 select 하세요.
		...
		Board board=null;
		board=dao.viewArticle(articleNo);
...
//		3. 2에서 얻어온 글정보를 출력하세요.
		...
		if(board != null) {
			...
			out.println("                />");
			out.println("                <p>");
			out.println("                  <span class=\"fw-bold\">" + board.getUsername() + "</span> <br />");
			...
			out.println(board.getContent().replace("\n", "<br>"));
			out.println("            </div>");
			...
			out.println("        location.href = \"/board1_servlet/delete?deleteno=\" + "+ articleNo+";");
			out.println("      });");
			out.println("    </script>");
			out.println("  </body>");
			out.println("</html>");
